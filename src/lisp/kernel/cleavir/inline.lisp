  ...
  (defmacro defcomparison (inline-name fixnum-op float-op generic-name)
    `(progn
       (define-with-contagion ,inline-name t (x y)
         ...
         ((,generic-name x y)))
       (define-cleavir-compiler-macro ,inline-name (&whole form x y &environment env)
         (let ((xt (form-type x env)) (yt (form-type y env)))
           (cond ((and (subtypep xt 'fixnum env) (subtypep yt 'fixnum env))
                  (list 'if (list ',fixnum-op x y) 't 'nil))
                 ((and (subtypep xt 'single-float env) (subtypep yt 'single-float env))
                  (list 'if (list ',float-op 'single-float x y) 't 'nil))
                 ((and (subtypep xt 'double-float env) (subtypep yt 'double-float env))
                  (list 'if (list ',float-op 'double-float x y) 't 'nil))
                 (t form))))))
...
(progn
  ...
  (define-cleavir-compiler-macro plusp (&whole form real &environment env)
    (let ((type (form-type real env)))
      (cond ((subtypep type 'fixnum env)
             `(if (cleavir-primop:fixnum-greater ,real 0) t nil))
            ((subtypep type 'single-float env)
             `(if (cleavir-primop:float-greater single-float ,real 0f0) t nil))
            ((subtypep type 'double-float env)
             `(if (cleavir-primop:float-greater double-float ,real 0d0) t nil))
            (t form)))))
...
(progn
  ...
  (define-cleavir-compiler-macro minusp (&whole form real &environment env)
    (let ((type (form-type real env)))
      (cond ((subtypep type 'fixnum env)
             `(if (cleavir-primop:fixnum-less ,real 0) t nil))
            ((subtypep type 'single-float env)
             `(if (cleavir-primop:float-less single-float ,real 0f0) t nil))
            ((subtypep type 'double-float env)
             `(if (cleavir-primop:float-less double-float ,real 0d0) t nil))
            (t form)))))
